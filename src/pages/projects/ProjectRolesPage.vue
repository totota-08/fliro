...
import { listenProjectMembers, updateProjectMemberRole, type ProjectMember } from '@/services/projectMembers'
...
const navItems = computed<DashboardNavItem[]>(() =>
  [
    ...
    { key: 'team', label: 'スレッド', to: { name: ROUTE_NAMES.projectThreads, params: { projectId: projectId.value } }, icon: 'team' },
    { key: 'roles', label: 'ロール', to: { name: ROUTE_NAMES.projectRoles, params: { projectId: projectId.value } }, icon: 'members' },
    { key: 'members', label: 'メンバー', to: { name: ROUTE_NAMES.projectMembers, params: { projectId: projectId.value } }, icon: 'members' },
    { key: 'settings', label: '設定', to: { name: ROUTE_NAMES.projectSettings, params: { projectId: projectId.value } }, icon: 'settings' },
...
async function evaluatePermissions() {
  ...
  const snap = await getDoc(doc(db, 'projects', projectId.value, 'members', user.value.uid))
...
<template>
  <div :class="['roles-shell', { 'roles-shell--sidebar-collapsed': !isSidebarOpen }]">
    <DashboardSidebar
      :open="isSidebarOpen"
      :nav-items="navItems"
      ...
      @close="closeSidebar"
    />
    <div v-if="isSidebarOpen" class="roles-shell__overlay" @click="closeSidebar" />

    <main class="roles-shell__main">
      <header class="roles-shell__topbar">
        <div class="roles-shell__topbar-left">
          <button type="button" class="roles-shell__menu-button" @click="toggleSidebar">
            <span class="sr-only">サイドバーを切り替え</span>
            <svg aria-hidden="true" class="roles-shell__menu-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div class="roles-content">
        <section class="roles-legend">
          ...
          <ul>
            ...
            <li><strong>member</strong> — タスク作成・更新、スレッド参加が可能</li>
          ...
          <ul class="roles-list">
            <li v-for="member in members" :key="member.userId" class="roles-item">
              <div class="roles-item__persona">
                <div class="avatar" aria-hidden="true">{{ (member.displayName || member.userId).slice(0, 2) }}</div>
                <div>
                  <p class="roles-item__name">{{ member.displayName || member.userId }}</p>
                  <p class="roles-item__meta">{{ member.email || member.userId }}</p>
                </div>
              </div>
              <div class="roles-item__selector">
                ...
                <select
                  ...
                  :value="member.role"
                  :disabled="!canEdit || member.role === 'owner' || updating === member.userId"
            ...
            <li v-if="!members.length" class="roles-item roles-item--empty">メンバーがいません。</li>
          </ul>
