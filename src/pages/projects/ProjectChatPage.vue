...
const taskMap = computed(() =>
  Object.fromEntries(
    tasks.value.map((task) => [
      task.id,
      task.threadName || task.title || "無題のタスク",
    ]),
  ),
);
...
const threadSettingsForm = ref<{ name: string; description: string }>({
  name: "",
  description: "",
});
...
const availableCommands: SlashCommand[] = [
  {
    key: "/private",
    label: "/private @user メッセージ",
    description: "指定ユーザーにのみ送信",
    insert: "/private @",
  },
...
const filteredTaskChannels = computed(() =>
  channels.value.filter((channel) => channel.type === "task"),
);
const customThreadChannels = computed(() =>
  channels.value.filter((channel) => channel.type === "custom"),
);

const composerPlaceholder = computed(
  () => `${currentChannel.value?.name || "このチャネル"}にメッセージを送信`,
);

const currentChannelMessages = computed(() =>
  messages.value.filter(
    (message) => (message.channelId || "general") === currentChannel.value?.id,
  ),
);

const currentThreadMessageCount = computed(
  () => currentChannelMessages.value.length,
);

const currentTaskDetail = computed(() => {
  if (!currentChannel.value || currentChannel.value.type !== "task")
    return null;
  return (
...
const currentTaskAssignee = computed(() => {
  ...
  return (
    getDisplayName(assigneeId) || currentTaskDetail.value?.assigneeName || ""
  );
});
...
const visibleChannelMessages = computed(() =>
  currentChannelMessages.value.filter((msg) => {
    if (
      msg.privateFor &&
      msg.privateFor !== user.value?.uid &&
      msg.senderId !== user.value?.uid
    )
      return false;
...
const lastMessageMeta = computed(() => {
  const map: Record<
    string,
    { id: string; senderId?: string; preview: string }
  > = {};
      ...
      if (channelId === activeChannelId.value) {
        ...
        if (notificationsEnabled.value && info.senderId !== user.value?.uid) {
          newMessagePreview.value =
            info.preview.length > 40
              ? `${info.preview.slice(0, 38)}…`
              : info.preview || "新着メッセージがあります";
...
const typingIndicator = computed(() => {
  const entries = Object.entries(typingUsers.value).filter(
  ...
  return matches
    ...
    .map((name) => {
      const member = projectMembers.value.find(
        (m) =>
          (m.nickname || m.fullName || m.displayName || "").toLowerCase() ===
          name.toLowerCase(),
      );
...
function toMillis(
  value: ChatMessage["createdAt"] | number | null | undefined,
): number | null {
...
const commandCandidates = computed(() =>
  availableCommands.filter((cmd) =>
    slashQuery.value
      ? cmd.key.toLowerCase().includes(slashQuery.value.toLowerCase())
      : true,
  ),
);
...
async function convertToTask(messageId: string, text: string) {
  ...
  const taskId = await createTask(
    projectId.value,
    { title: text, hasThread: true, threadName: text },
    user.value.uid,
  );
  await update(
    dbRef(database, `projects/${projectId.value}/realtimeChat/${messageId}`),
    { linkedTaskId: taskId },
  );
...
function toggleReactionPicker(messageId: string) {
  openReactionFor.value =
    openReactionFor.value === messageId ? null : messageId;
}
...
function detectMessageType(
  text: string,
): "important" | "question" | "announcement" | "success" | "normal" {
  ...
  if (
    ...
    lowerText.includes("教えて")
  ) {
    return "question";
  }
  if (
    lowerText.includes("お知らせ") ||
    lowerText.includes("告知") ||
    lowerText.includes("開催")
  ) {
    return "announcement";
  }
  if (
    lowerText.includes("完了") ||
    ...
    <div class="content-wrapper">
      ...
      <aside class="threads-panel">
        ...
        <div class="threads-section">
          ...
          >
            <UserAvatar :name="channel.name" :size="24" class="thread-avatar" />
            <span class="thread-name">{{ channel.name }}</span>
            <span v-if="unreadChannels[channel.id]" class="thread-badge">{{
              unreadChannels[channel.id] ? "1" : ""
            }}</span>
          </div>
          <p v-if="!filteredTaskChannels.length" class="empty-text">
            No tasks found
          </p>
          ...
          >
            ...
            <div class="thread-info">
              <span class="thread-name">{{ channel.name }}</span>
              <span v-if="channel.description" class="thread-meta">{{
                channel.description
              }}</span>
            </div>
            <span v-if="unreadChannels[channel.id]" class="thread-badge">{{
          ...
          <p v-if="!customThreadChannels.length" class="empty-text">
            No custom threads
          </p>
        </div>
      </aside>
      ...
      <main class="main-chat">
        <header class="chat-header">
          <div class="channel-title">
            <span>#</span>
            <span class="channel-name" :title="currentChannel?.name">{{
              currentChannel?.name
            }}</span>
          </div>
            ...
            >
              <div class="message-header">
                ...
                <div class="message-info">
                  ...
                  <span
                    v-if="message.linkedTaskId || message.isTask"
                    class="message-badge badge-task"
                    >タスク</span
                  >
                  <span
                    ...
                    class="message-badge"
                    :class="
                      getMessageTypeLabel(detectMessageType(message.text))
                        ?.class
                    "
                  >
                    {{
              ...
              <div class="message-actions">
                ...
                <button
                  class="action-btn"
                  @click="convertToTask(message.id, message.text)"
                  title="タスク化"
                >
                  <svg
                    width="18"
              ...
              <!-- Reaction Picker -->
              <div
                v-if="openReactionFor === message.id"
              ...
              <div v-if="message.linkedTaskId" class="linked-task-info">
                ...
                <span
                  >紐付け:
                  {{
                    taskMap[message.linkedTaskId] || message.linkedTaskId
                  }}</span
                >
              </div>
            </div>
        ...
        <div class="input-area">
          <div v-if="replyingTo" class="reply-banner">
            <span
              >返信中: <strong>{{ displayNameFor(replyingTo) }}</strong></span
            >
      ...
      <aside class="task-panel">
        <div v-if="currentTaskDetail" class="task-panel__card">
          ...
          <p class="task-panel__desc">
            {{ currentTaskDetail.description || "説明が設定されていません。" }}
          </p>
          <dl class="task-panel__meta">
            <div>
              <dt>ステータス</dt>
              <dd class="status-pill">
                {{ currentTaskDetail.status || "未設定" }}
              </dd>
            </div>
            <div>
              <dt>担当</dt>
        ...
        <div class="thread-modal">
          ...
          <form class="thread-modal__form" @submit.prevent="submitNewThread">
            <label>
              スレッド名
              <input
                v-model="newThreadForm.name"
                type="text"
                placeholder="デザインレビュー"
                required
              />
            </label>
            <label>
              説明 (任意)
              <textarea
                v-model="newThreadForm.description"
                rows="3"
                placeholder="スレッドの背景を入力"
              ></textarea>
            </label>

            <footer>
              <button type="button" class="ghost" @click="closeNewThreadModal">
                キャンセル
              </button>
              <button type="submit" :disabled="!newThreadForm.name.trim()">
                作成
              </button>
            </footer>
          </form>
      ...
      <div v-if="threadSettingsOpen" class="thread-modal-overlay">
        <div class="thread-modal">
          ...
          <form class="thread-modal__form" @submit.prevent="saveThreadSettings">
            ...
            <footer>
              <button type="button" class="ghost" @click="closeThreadSettings">
                キャンセル
              </button>
              <button type="submit" :disabled="!threadSettingsForm.name.trim()">
                保存
              </button>
            </footer>
          </form>
          <div class="thread-settings__danger">
            <p>
              このスレッドを削除すると、メッセージ履歴は保持されますがチャネル一覧からは消えます。
            </p>
            <button type="button" class="danger" @click="deleteCurrentThread">
              スレッドを削除
            </button>
          </div>
        </div>
