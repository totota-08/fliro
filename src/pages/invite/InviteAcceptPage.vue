...
async function loadInviteDetails() {
  ...
  try {
    ...
    const maxUses = typeof invite.maxUses === 'number' && invite.maxUses > 0 ? invite.maxUses : null
    ...
    projectPreview.value = {
      ...
      expiresAtLabel: expiresMillis ? formatDate(new Date(expiresMillis)) : '有効期限なし',
      remainingUses,
    }
...
async function ensureAuthenticatedUser() {
  ...
  if (authMode.value === 'signup') {
    ...
    await registerCredentials({ email: emailInput.value.trim(), password: passwordAuthInput.value.trim() })
  } else {
      ...
      <template v-else>
        ...
        <div v-if="!user" class="auth-card">
          <div class="auth-toggle">
            <button type="button" :class="{ 'is-active': authMode === 'signup' }" @click="authMode = 'signup'">新規登録</button>
            <button type="button" :class="{ 'is-active': authMode === 'login' }" @click="authMode = 'login'">ログイン</button>
          </div>
          ...
          <label>
            <span>パスワード</span>
            <input v-model="passwordAuthInput" type="password" placeholder="6文字以上" />
          </label>

          <label v-if="authMode === 'signup'">
            <span>パスワード確認</span>
            <input v-model="passwordAuthConfirm" type="password" placeholder="もう一度入力" />
          </label>
        </div>
        ...
        <div class="actions">
          ...
          <AppButton variant="primary" :loading="joinProcessing || loading" :disabled="!canJoin" @click="handleJoin">
            {{ user ? '参加する' : authMode === 'signup' ? 'アカウントを作成して参加' : 'ログインして参加' }}
          </AppButton>
        </div>
      </template>
    </section>
