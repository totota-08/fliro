...
export function useSlashCommands({
  ...
  async function sendBotMessage(
    text: string,
    options?: { privateFor?: string | null },
  ) {
    setBotTyping?.(true);
    try {
    ...
    if (lower.startsWith("/private")) {
      ...
      if (!match) {
        await sendBotMessage(
          "private コマンドの形式: /private @ユーザー 本文",
      ...
      if (!targetUser) {
        await sendBotMessage(`${targetName} さんが見つかりません`, {
          privateFor: user.value?.uid || null,
        });
        return true;
      }
      if (!body) {
        await sendBotMessage("メッセージを入力してください", {
          privateFor: user.value?.uid || null,
        });
        return true;
      }
      ...
      await sendBotMessage(
        `${targetName} さんへのプライベートメッセージを送信しました`,
        {
    ...
    if (lower.startsWith("/news")) {
      try {
        ...
        const items = (data?.hits || [])
          .slice(0, 3)
          .map((hit: any, idx: number) => `${idx + 1}. ${hit?.title}`);
        await sendBotMessage(
          items.length
            ? `今日のニュース\n${items.join("\n")}`
            : "ニュースを取得できませんでした",
