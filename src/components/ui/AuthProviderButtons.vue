...
<template>
  <div class="provider-buttons">
    <button
      ...
      :disabled="props.loading !== null && props.loading !== undefined && props.loading !== provider.id"
      @click="emit('select', provider.id)"
    >
      <span v-if="provider.icon" class="provider-buttons__icon" :data-icon="provider.icon" />
      {{
        props.loading === provider.id ? `${provider.label}中...` : provider.label
      }}
    </button>
  </div>
</template>
